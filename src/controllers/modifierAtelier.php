<?php 
  ...
  function modifierAtelier($data, $image, $id){
    ...
    if($validationInputs[0]){
        if($image !== null){
            ...
            if(!$validationImage[0]){
                if($data['image'] !== ""){
                    $oldFilename = __ROOT__."/images/" . $data['image'];
                    unlink($oldFilename);
                }
                ...
                if($dataAtelier !== null){
                  ...
                  $data['modifie'] = true; //Il s'agit ici d'un indice à false donc creation et true si modifié';

                  foreach ($dataAtelier as $key => $value) {
                    if($value['id'] == $id){
                        $dataAtelier[$key]['titre'] = $data['titre'];
                        $dataAtelier[$key]['description'] = $data['description'];
                        $dataAtelier[$key]['date_debut'] = $data['date_debut'];
                        $dataAtelier[$key]['heureDebut'] = $data['heureDebut'];
                        $dataAtelier[$key]['minDebut'] = $data['minDebut'];
                        $dataAtelier[$key]['heureDuree'] = $data['heureDuree'];
                        $dataAtelier[$key]['minutesDuree'] = $data['minutesDuree'];
                        $dataAtelier[$key]['nombre_places'] = $data['nombre_places'];
                        $dataAtelier[$key]['prix'] = $data['prix'];
                        $dataAtelier[$key]['heure_debut'] = $data['heure_debut'];
                        $dataAtelier[$key]['duree'] = $data['duree'];
                        $dataAtelier[$key]['date_ajout'] = $data['date_ajout'];
                        $dataAtelier[$key]['modifie'] = $data['modifie'];
                    }
                  }
                ...
                if($dataAtelier !== null){
                    //On recupere l'ancien nom de l'image
                ...
                //   array_unshift($dataAtelier, $data);
                  ...
                  //On dit que la verification est ok via la variable du début
                  array_push($validationInputs, '<div class="container alert alert-success col-12 mb-5">Atelier modifié avec succès</div>');
                  return $validationInputs;
                  exit();
                }else{
                  $validationInputs[0] = false;
